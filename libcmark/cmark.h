...
#define CMARK_NODE_INLINE_HTML CMARK_NODE_HTML_INLINE
     ...
     *     }
     *
     * Iterators will never return `EXIT` events for leaf nodes, which are
     * nodes of type:
     *
     * * CMARK_NODE_HTML_BLOCK
     * * CMARK_NODE_THEMATIC_BREAK
     ...
     * * CMARK_NODE_HTML_INLINE
     *
     * Nodes must only be modified after an `EXIT` event, or an `ENTER` event
     * for leaf nodes.
     */
     ...
     * (`CMARK_EVENT_ENTER`, `CMARK_EVENT_EXIT` or `CMARK_EVENT_DONE`).
     */
    CMARK_EXPORT
     ...
     * 0 on failure.
     */
    CMARK_EXPORT int cmark_node_set_user_data (cmark_node *node,
                                               void *user_data);
     ...
     * 0 on failure.
     */
    CMARK_EXPORT int cmark_node_set_literal (cmark_node *node,
                                             const char *content);
...
#define cmark_node_set_header_level cmark_node_set_heading_level
     ...
     * error.
     */
    CMARK_EXPORT int cmark_node_set_heading_level (cmark_node *node,
                                                   int level);
    ...
    /** Returns the list delimiter type of 'node', or `CMARK_NO_DELIM` if
     * 'node' is not a list.
     */
    ...
    /** Returns starting number of 'node', if it is an ordered list, otherwise
     * 0.
     */
     ...
     */
    CMARK_EXPORT int cmark_node_set_fence_info (cmark_node *node,
                                                const char *info);
     ...
     * 0 on failure.
     */
    CMARK_EXPORT int cmark_node_set_title (cmark_node *node,
                                           const char *title);
    ...
    /** Sets the literal text to render "on exit" for a custom 'node'.
        ...
        Returns 1 on success 0 on failure.
     */
    CMARK_EXPORT int cmark_node_set_on_exit (cmark_node *node,
                                             const char *on_exit);
     ...
     */
    CMARK_EXPORT int cmark_node_insert_after (cmark_node *node,
                                              cmark_node *sibling);
     ...
     * Returns 1 on success, 0 on failure.
     */
    CMARK_EXPORT int cmark_node_replace (cmark_node *oldnode,
                                         cmark_node *newnode);

    /** Adds 'child' to the beginning of the children of 'node'.
     * Returns 1 on success, 0 on failure.
     */
    CMARK_EXPORT int cmark_node_prepend_child (cmark_node *node,
                                               cmark_node *child);

    /** Adds 'child' to the end of the children of 'node'.
     * Returns 1 on success, 0 on failure.
     */
    CMARK_EXPORT int cmark_node_append_child (cmark_node *node,
                                              cmark_node *child);
     ...
     */
    CMARK_EXPORT
    void cmark_parser_feed (cmark_parser *parser, const char *buffer,
                            size_t len);
